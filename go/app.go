...
func recentHandler(w http.ResponseWriter, r *http.Request) {
	...
	rows, err = dbConn.Query("SELECT * FROM memos WHERE is_private=0 ORDER BY id DESC LIMIT ? OFFSET ?", memosPerPage, memosPerPage*page)
	if err != nil {
		serverError(w, err)
		return
	}
...
func mypageHandler(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := dbConn.Query("SELECT id, content, is_private, created_at, updated_at FROM memos WHERE user=? ORDER BY id DESC", user.Id)
	if err != nil {
		serverError(w, err)
		return
	}
...
func memoHandler(w http.ResponseWriter, r *http.Request) {
	...
	rows, err = dbConn.Query("SELECT id, content, is_private, created_at, updated_at FROM memos WHERE user=? "+cond+" ORDER BY id", memo.User)
	if err != nil {
		serverError(w, err)
		return
	}
